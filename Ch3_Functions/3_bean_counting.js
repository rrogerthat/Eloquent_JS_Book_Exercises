...
// found at position string.length - 1. In other words, a two-character string has length 2, and its characters 
...
function countChar(input, item) {
    let pointer = 0;
    let count = 0;

    function helper() {
        if (input[pointer] === item) count++;
        while (pointer < input.length) {
            pointer++;
            return helper();
        }
